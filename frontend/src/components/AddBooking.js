...
//validations to form
validate=(name,email,route,date,Time,from,to,seatno)=>{
  const errors = setErrors(name,email,route,date,Time,from,to,seatno);
...
window.location=`/addpayment`;
  ...
  <Card.Header><h4>Seat Rerservation</h4></Card.Header>
  <Card.Body>
  <Form >	
  <Card.Text>
...
{this.state.errors.email && (
  ...
  name="route"> required
    ...
    {this.state.errors.route && (
    <div className ="text-danger">{this.state.errors.route
    }</div>
  )}
...
{this.state.errors.seatno && (
    <div className ="text-danger">{this.state.errors.seatno}</div>)}
